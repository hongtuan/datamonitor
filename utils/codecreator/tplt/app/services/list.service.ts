...
export class ___ComponentName___Service {
  ...
  private apiUrl = '/api/models/';  // URL to web api
  private getModelUrl = '/api/userhislist/';  // URL to web api

  constructor(private http: Http) { }

  getModels(userInfo:any): Observable<any[]> {
    //需要按用户过滤列表.
    return this.http.post(this.getModelUrl,userInfo,this.jsonRequestOptions)
      .map(this.extractData).catch(this.handleError);
  }

  addModel(formData:any,userInfo:any):Observable<any> {
  ...
  deleteModel(hid:string):Observable<any> {
